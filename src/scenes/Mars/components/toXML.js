...
const toXML = (samples, usercode) => {
  ...
  for (let i = 0; i < samples.length; i++) {
    //create sample
    let sampleNode = document.createElementNS(
      "http://app.geosamples.org",
      "sample"
    );
    sampleNode
      .appendChild(
        document.createElementNS("http://app.geosamples.org", "user_code")
      )
    ...
    sampleNode
      .appendChild(
        document.createElementNS("http://app.geosamples.org", "elevation_unit")
      )
